...
thresholds_green = [(39, 11, -128, -10, -57, 31)] # generic_green_thresholds
thresholds_orange = [(98, 11, 5, 114, -23, 121)]
...
while(True):
    ...
    #for blob in img.find_blobs([thresholds_green[0]], pixels_threshold=100, area_threshold=50, merge=True):#找綠色方塊
        ...
        #time.sleep_ms(10)
    for blob in img.find_blobs([thresholds_orange[0]], pixels_threshold=200, area_threshold=200, merge=True):#找綠色方塊
        # These values depend on the blob not being circular - otherwise they will be shaky.
        if blob.elongation() > 0.5:
            img.draw_edges(blob.min_corners(), color=(255,0,0))
            img.draw_line(blob.major_axis_line(), color=(0,255,0))
